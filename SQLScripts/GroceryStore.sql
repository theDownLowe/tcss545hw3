...
CREATE TRIGGER before_department_update
    ...
    FOR EACH ROW
BEGIN
	 SELECT CONCAT("Updating: ", OLD.Name);
END $$
DELIMITER ;
...
CREATE USER 'distributionManager' IDENTIFIED by 'password';
CREATE USER 'executive' IDENTIFIED BY 'password'

GRANT ALL PRIVILEGES ON grocerystore.* TO 'executive';
GRANT ALL PRIVILEGES ON grocerystore.Distribution TO 'distributionManager';
...
CREATE FUNCTION welcome (s VARCHAR(60))
    RETURNS VARCHAR(60) DETERMINISTIC
    RETURN "Hello and Welcome to the Grocery Store :)";
END
